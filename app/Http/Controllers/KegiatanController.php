...
class KegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('kegiatan.kegiatan_index', compact('user_access', 'direktorats'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->q == 'add') {
                ...
                DtKegiatan::where('id', $request->eid)
                    ->update([
                ...
                DtKegiatan::where('id', $request->d_id)
                    ->update([
                        'is_trash' => 1,
                        'updated_by' => auth()->user()->id
                    ]);
            } elseif ($request->q == 'restore') {
                DtKegiatan::where('id', $request->delidtxt)
                    ->update([
                        'is_trash' => 0,
                        'updated_by' => auth()->user()->id
                    ]);
