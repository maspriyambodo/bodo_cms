                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning d-flex align-items-center p-5 mb-10">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <span>restoring data subdirektorat causes the user to be able to access the system again</span>
                        </div>
                    </div>
                </div>
...
<script>
    function restoreData(id_direktorat) {
        ...
        $.ajax({
            url: 'subdit/edit/' + id_direktorat,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                if (data.success) {
                    $('input[name="delidtxt"]').val(data.dt_subdit['id']);
                    $('#restoreModalLabel').html('Restore subdirektorat <strong>' + data.dt_subdit.nama + '</strong>');
        ...
        if (validator_resotre) {
            validator_resotre.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch("{{ route('subdit.store', 'q=restore') }}", {
                        method: 'POST',
