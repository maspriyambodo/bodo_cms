...
<script>
    function deleteData(id_menu) {
        ...
        $.ajax({
            url: 'user-groups/edit/' + id_menu,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                if (data.success) {
                    $('input[name="d_id"]').val(data.dt_usergrup.id);
                    $('#deltxt').html('Are you sure you want to delete user group <strong>' + data.dt_usergrup['name'] + '</strong>?');
        ...
        if (validator3) {
            validator3.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch('user-groups/store/?q=delete', {
                        method: 'POST',
                        body: formData
                    })
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    Swal.fire({
                                        text: "error while insert data, errcode: 01030023",
                                        icon: "error",
                                        buttonsStyling: !1,
                                        confirmButtonText: "OK",
                                        allowOutsideClick: false,
                            ...
                            .catch((error) => {
                                Swal.fire({
                                    text: "error while insert data, errcode: 01030024",
                                    icon: "error",
                                    buttonsStyling: !1,
                                    confirmButtonText: "OK",
                                    allowOutsideClick: false,
