                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning d-flex align-items-center p-5 mb-10">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <span>restoring data user group causes the user to be able to access the system again</span>
                        </div>
                    </div>
                </div>
    ...
    function restoreData(id_menu) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    $('input[name="delidtxt"]').val(data.dt_usergrup['id']);
                    $('#restoreModalLabel').html('Restore user group <strong>' + data.dt_usergrup.name + '</strong>');
        ...
        if (validator_resotre) {
            validator_resotre.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch('user-groups/store/?q=restore', {
                        method: 'POST',
