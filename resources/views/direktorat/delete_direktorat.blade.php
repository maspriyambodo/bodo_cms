    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="deleteModalLabel">Delete Direktorat</h1>
...
@push('scripts')
<script>
    function deleteData(id_direktorat) {
        Swal.fire({
            title: 'memuat data...',
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    $('input[name="d_id"]').val(data.dt_direktorat.id);
                    $('#deltxt').html('Are you sure you want to delete direktorat <strong>' + data.dt_direktorat['nama'] + '</strong>?');
        ...
        if (validator3) {
            validator3.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch("{{ route('direktorat.store', 'q=delete') }}", {
                        method: 'POST',
                        body: formData
                    })
