                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning d-flex align-items-center p-5 mb-10">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <span>restoring data direktorat causes the user to be able to access the system again</span>
                        </div>
                    </div>
                </div>
...
@push('scripts')
<script>
    function restoreData(id_direktorat) {
        Swal.fire({
            title: 'memuat data...',
        ...
        $.ajax({
            url: 'direktorat/edit/' + id_direktorat,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                if (data.success) {
                    $('input[name="delidtxt"]').val(data.dt_direktorat['id']);
                    $('#restoreModalLabel').html('Restore direktorat <strong>' + data.dt_direktorat.nama + '</strong>');
        ...
        if (validator_resotre) {
            validator_resotre.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch("{{ route('direktorat.store', 'q=restore') }}", {
                        method: 'POST',
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        ...
                                        $('#table-direktorat').DataTable().ajax.reload();
