                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning d-flex align-items-center p-5 mb-10">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <span>restoring data kecamatan causes the user to be able to access the system again</span>
                        </div>
                    </div>
                </div>
...
<script>
    function restoreData(id_menu) {
        ...
        $.ajax({
            url: 'kelurahan/edit/' + id_menu,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                if (data.success) {
                    $('input[name="delidtxt"]').val(data.dt_kelurahan['id_kelurahan']);
                    $('#restoreModalLabel').html('Restore kecamatan <strong>' + data.dt_kelurahan.nama + '</strong>');
        ...
        if (validator_resotre) {
            validator_resotre.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch('kelurahan/store/?q=restore', {
                        method: 'POST',
