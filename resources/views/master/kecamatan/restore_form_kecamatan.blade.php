    ...
    function restoreData(id_menu) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    $('input[name="delidtxt"]').val(data.dt_kecamatan['id_kecamatan']);
                    $('#restoreModalLabel').html('Restore kecamatan <strong>' + data.dt_kecamatan.nama + '</strong>');
        ...
        if (validator_resotre) {
            validator_resotre.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    fetch('kecamatan/store/?q=restore', {
                        method: 'POST',
